...
export async function createConversation(
  ...
  const dialogue: Dialogue = [
    ...
    { sender: user2.id, time: i(s, 90), text: 'ROP stands for "Return-Oriented Programming." It\'s a way to exploit buffer overflows by chaining together existing code snippets from the program. It\'s like building a puzzle with the program\'s own pieces.' },
    { sender: user1.id, time: i(s, 150), text: 'Got it! So, you craft a sequence of return addresses to execute the actions you want, like gaining control of the program or escalating privileges. Sounds powerful!' },
    { sender: user2.id, time: i(s, 220), text: 'Exactly! It involves constructing a series of return addresses that point to gadgets—small code sequences in the program—that perform specific tasks. By chaining these gadgets, we can achieve arbitrary code execution.' },
    { sender: user2.id, time: i(s, 280), text: 'But it\'s not always easy. Finding the right gadgets and constructing the chain can be challenging, and it depends on the program\'s memory layout and libraries.' },
